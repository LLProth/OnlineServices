...
{
  public partial class IRConfirmation : System.Web.UI.Page
  {
    protected void Page_Load(object sender, EventArgs e)
    {
      if (this.Session.Contents["IR_ID"] == null)
        Response.Redirect("IRCompanySelect.aspx");
      string inIRID = this.Session.Contents["IR_ID"].ToString();
      
      HtmlHelper.SetupLinkFromConfigForPopupAnchor(anchorOFROI, "Link_FROI");


      
      //IncidentReportWorker thisWorker = new IncidentReportWorker(Convert.ToInt32(inIRID));
        //Session.Contents["IRW"]

      if (Session.Contents["IRW"] != null)
      {
          IncidentReportWorker thisWorker = (IncidentReportWorker)Session.Contents["IRW"];
